...
# 🚨 IMPORTANTE: Se elimina la importación de Migrate y upgrade para forzar db.create_all()
...
db = SQLAlchemy(app)
# 🚨 ELIMINADA LA INSTANCIA DE MIGRATE
...
# 5. USAMOS LAS FUNCIONES DE SELECTOR DIRECTAMENTE EN BABEL (Método Alternativo)
try:
...
        # Esta lógica solo se ejecuta en el primer despliegue después de purgar
        # para asegurar que todas las tablas, incluyendo phone_number, existan.
        try:
            # 🚨 ADVERTENCIA: db.drop_all() borra TODOS los datos. Usar solo en desarrollo o si la DB es nueva.
            # Lo comentamos para que solo haga create_all() por si ya se purgo externamente.
            # db.drop_all() 
            db.create_all() 
            logging.info("DB tables created successfully (db.create_all() executed).")
        
...
            
        except Exception as e:
            logging.error(f"CRITICAL ERROR during DB initialization: {e}")
            sys.exit(1) # Salir si la DB no se puede crear (problema de URL o credenciales)
